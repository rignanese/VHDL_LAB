...
The first exercise you can do is to build a clocked version of the half_adder design. To do that:
...
To help you in the debugging, there is my version on the exercise in the clocked_half_adder folder

The second exercise consists in creating a sngle bit full adder. To do that you can simply add the carry_in port to the half_adder design and compute the new logic: 
...
In the third exercise we are going to create a 2bit counter. The idea is to generate an output that increases every time a rising edge of the clock is received. When a reset is asserted, the counter output will be 0.
...
- the architecture is composed by two concurrent processes. The first one is in charge of increasing u_count (and control the overflow of it), while the second one is in charge of putting the value of `u_count` into the `count` port (this second process is just a cuncurrent statement)
- create a sequential process with clk and rst in the sensitivity list
- first of all define the behavior of the device when the rst is 1 `if rst = '1' then` your `unsigned` signal musto go to 0
...
- before ending the architecture, add the cocnurrent statement `count<=std_logic_vector(u_count)`. In this way, the value of `u_count` is converted in a std_logic_vector and assigned to `count`
...
To help you in the debugging, there is my version on the excerise in the 2bit_counter folder of this repository.

The fourth exercise is to build a 4bit ripple-carry adder (https://en.wikipedia.org/wiki/Adder_(electronics)#Ripple-carry_adder). We can use the structural approach and connect 4 single bit full-adders accordingly with this figure:
